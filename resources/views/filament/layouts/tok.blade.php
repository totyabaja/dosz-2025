@php
    ...
    $menuItems = [
        (object) [
            'route' => 'public.home', // TODO
            'name' => 'Szervezet',
            'subs' => [
                (object) [
                    'route' => 'public.to.pages',
                    'params' => [
                        'to_slug' => $to->slug,
                        'slug' => 'bemutatkozas',
                    ],
            ...
            <div class="top-0 z-50 hidden py-2 text-sm bg-gray-100 md:block">
                <div class="flex items-center justify-between px-4">
                    ...
                    <span class="flex items-center space-x-6 text-gray-600">
                        ...
                        <div class="flex items-center space-x-2">
                            <img src="{{ Storage::url($settings->site_favicon) }}" style="max-height: 40px;">
                            <a href="/">
                                DOSZ.HU
                            </a>
                        </div>
                ...
                <nav class="flex items-center justify-between w-full gap-2 shadow navbar rounded-box">
                    ...
                    <div class="navbar-start max-md:w-1/4">
                        <a class="text-xl font-semibold no-underline link text-base-content link-neutral"
                            href="{{ route('public.to', $to_slug) }}">
                            <img alt="{{ $settings->brand_name }} logo" src="{{ $to->getFilamentAvatarUrl() }}"
                    ...
                    <div class="hidden navbar-center md:flex">
                        <ul class="p-0 font-medium menu menu-horizontal">
                            @foreach ($menuItems as $menuItem)
                                ...
                                @else
                                    <li><a
                                            href="{{ route($menuItem->route, ['to_slug' => $to->slug]) }}">{{ $menuItem->name }}</a>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
                        ...
                        <div class="px-2 pt-4 drawer-body">
                            <ul
                                ...
                                @foreach ($menuItems as $menuItem)
                                    ...
                                    @else
                                        <li>
                                            <a href="{{ route($menuItem->route, ['to_slug' => $to->slug]) }}">
